...

{
	...
	if ( ! verbose )
	{
		printf( "   visual  x  bf lv rg d st  r  g  b a  ax dp st accum buffs  "
				"ms \n" );

		printf( " id dep cl sp sz l  ci b ro sz sz sz sz bf th cl  r  g  b  a "
				"ns b\n" );
	...
	*/
	...
	{ /* verbose output. */
		...
		for ( i = 1; i <= maxpf; i++ )
		{
			...
			printf( "    rgba: redSize=%d greenSize=%d blueSize=%d "
					"alphaSize=%d\n",
			...
			printf( "    accum: redSize=%d greenSize=%d blueSize=%d "
					"alphaSize=%d\n",
...
main( int argc, char** argv )
{
	...
	while ( --argc )
	{
		...
		{
			printf( "Usage: wglinfo [-v] [-t] [-m] [-h] [-dispay <dname>] "
					"[-nfbc] [-fpcinfo]\n" );
			printf( "        -v: Print visuals info in verbose form.\n" );
			printf( "        -t: Print verbose table (not implemented on "
					"Win32)\n" );
			printf( "        -m: Don't print mid table headers (in long "
					"tables). (not on Win32)\n" );
			printf( "        -display <dname>: Print GLX visuals on specified "
					"server. (not on Win32)\n" );
			printf( "        -nfbc: Don't use fbconfig extension (not "
					"available on Win32)\n" );
			printf( "        -fbcinfo: print out additional fbconfig "
					"information (not on Win32)\n" );
	...
	while ( *s )
	{
		...
		if ( *s == ' ' )
		{
			...
			{ /* zoinks! last one terminated in a space! */
				t[i - 1] = '\0';
			}
		}
