...
init( void )
{
	...
	glClearColor(
			colors[2].diff[0], colors[2].diff[1], colors[2].diff[2], 1.0F );
...
setupDIB( HDC hDC )
{
	...
	if ( hBitmap == NULL )
	{
		(void)MessageBox( WindowFromDC( hDC ), "Failed to create DIBSection.",
...
setupPalette( HDC hDC )
{
	...
	DescribePixelFormat(
			hDC, pixelFormat, sizeof( PIXELFORMATDESCRIPTOR ), &pfd );
...
setupPixelFormat( HDC hDC )
{
	...
	if ( retVal != TRUE )
	{
		(void)MessageBox( WindowFromDC( hDC ), "Failed to set pixel format.",
				"OpenGL application error", MB_ICONERROR | MB_OK );
		exit( 1 );
	}
}
		...
		int nCmdShow )
{
	...
	hWnd = CreateWindow( className, /* Window class's name */
